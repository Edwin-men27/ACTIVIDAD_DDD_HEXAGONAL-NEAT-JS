...
import {
  AplicacionNotFoundException,
  NombreAplicacionDuplicadoException,
  VersionDuplicadaException,
  VersionNoValidaException,
} from '../../../domain/aplicaciones/exceptions';
...
export class DomainExceptionFilter implements ExceptionFilter {
    ...
    const map = new Map<Function, { status: number; message: string }>([
      ...
      [AplicacionNotFoundException, { status: HttpStatus.NOT_FOUND, message: exception.message }],
      [NombreAplicacionDuplicadoException, { status: HttpStatus.CONFLICT, message: exception.message }],
      [VersionDuplicadaException, { status: HttpStatus.BAD_REQUEST, message: exception.message }],
      [VersionNoValidaException, { status: HttpStatus.BAD_REQUEST, message: exception.message }],
    ]);
